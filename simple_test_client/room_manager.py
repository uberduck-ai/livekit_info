...
async def run_room(
    ...
    room_name: str,
    script: Optional[TestScript] = None,
    listener_mode: bool = False,
) -> None:
    """Run the room and handle participant connections and disconnections."""
    global publish_source
    logging.info("Running room %s", room_name)

    @room_instance.on("participant_disconnected")
    ...
    token = (
        api.AccessToken()
        .with_identity("python-listener" if listener_mode else "python-publisher")
        .with_name("Python Listener" if listener_mode else "Python Publisher")
        .with_grants(
            api.VideoGrants(
                room_join=True,
                room=room_name,
                can_publish=not listener_mode,
            )
        )
        .to_jwt()
    )
    ...
    try:
        ...
        logging.info("connected to room %s", room_instance.name)
    ...
    if not listener_mode:
        ...
        publish_track = rtc.LocalAudioTrack.create_audio_track(
            "publish", publish_source
        )
        publish_options = rtc.TrackPublishOptions()
        publish_options.source = rtc.TrackSource.SOURCE_MICROPHONE
        publication = await room_instance.local_participant.publish_track(
            publish_track, publish_options
        )
...
async def cleanup(room_instance: rtc.Room):
